...
bin/ci-smoke.sh — SpectraMind V50 CI/Kaggle-friendly smoke test (ultimate)
...
–outdir         Outputs directory (default: outputs/diagnostics)

–timeout        Per-step timeout if timeout exists (default: 600)

–hydra “”      Extra Hydra overrides for train/predict (quoted)
...
–log           Log file (default: logs/ci/ci-smoke-.log)
...
• Appends a structured line to logs/v50_debug_log.md for audit trails.
...
–fix-lock) FIX_LOCK=1 ;;
–deterministic) DETERMINISTIC=”${2:?}”; shift ;;
...
[[ “$DEVICE” == “gpu” ]] && warn “GPU device requested but CUDA not detected; continuing on CPU.”
fi

––––– Step: version alignment (non-fatal gate) –––––

if [[ -f “bin/version_tools.py” ]]; then
run “version validate” python3 bin/version_tools.py –validate || mark_fail “version-validate”
fi
...
# prefer quick/fast flag if available; otherwise fallback to generic selftest
if ${CLI_BIN} selftest –help 2>/dev/null | grep -qi – “–quick”; then
run “spectramind selftest –quick” ${CLI_BIN} selftest –quick || mark_fail “cli-selftest”
else
run “spectramind selftest” ${CLI_BIN} selftest || mark_fail “cli-selftest”
fi
fi
fi
...
Light path: always disables UMAP/t-SNE for speed & headless stability

run “diagnose smoothness” ${CLI_BIN} diagnose smoothness –outdir “$OUTDIR” || mark_fail “diagnose-smoothness”
if ${CLI_BIN} diagnose dashboard –help 2>/dev/null | grep -q – “–no-umap”; then
run “diagnose dashboard (light)” ${CLI_BIN} diagnose dashboard –no-umap –no-tsne –outdir “$OUTDIR” || mark_fail “diagnose-dashboard”
else
run “diagnose dashboard (default)” ${CLI_BIN} diagnose dashboard –outdir “$OUTDIR” || mark_fail “diagnose-dashboard”
fi
fi
...
run “tiny training run” ${CLI_BIN} train ${TRAIN_OVR} –device “$DEVICE” –outdir “outputs/checkpoints/smoke${DEVICE}” || mark_fail “tiny-train”
mkdir -p outputs/predictions
PRED_OVR=””
[[ -n “$HYDRA_OVERRIDES” ]] && PRED_OVR=”${HYDRA_OVERRIDES}”
if ${CLI_BIN} predict –help 2>/dev/null | grep -q – “–out-csv”; then
run “predict (stub CSV)” ${CLI_BIN} predict ${PRED_OVR} –out-csv “outputs/predictions/_smoke_submission.csv” || mark_fail “predict”
else
run “predict (default)” ${CLI_BIN} predict ${PRED_OVR} || mark_fail “predict”
fi
fi
fi
...
local status=”$1”
local ts_iso git_short cfg_hash tag pred bundle notes
...
tag=””; pred=””; bundle=”_”
notes=“mode=$([[ $DEEP -eq 1 ]] && echo deep || echo fast);device=${DEVICE};outdir=${OUTDIR};status=${status}”
mkdir -p logs
printf ‘[%s] cmd=ci-smoke git=%s cfg_hash=%s tag=%s pred=%s bundle=%s notes=”%s”\n’ 
“$ts_iso” “$git_short” “$cfg_hash” “$tag” “$pred” “$bundle” “$notes” >> logs/v50_debug_log.md
}
...
# failed steps as JSON array
if ((${#FAIL_STEPS[@]}==0)); then
printf ’“failed_steps”: [], ’
else
printf ‘“failed_steps”: [’
for i in “${!FAIL_STEPS[@]}”; do
printf ‘”%s”%s’ “${FAIL_STEPS[$i]}” $([[ $i -lt $((${#FAIL_STEPS[@]}-1)) ]] && echo ‘,’ || echo ‘’)
done
printf ’], ’
fi
