...
bin/selftest.sh — SpectraMind V50 quick/deep health check (ultimate, upgraded)
...
• (Optional) JSON result for CI dashboards + structured log line append
...
[–device <cpu|gpu>] [–log ] [–quiet]

[–no-cli] [–no-dvc] [–no-cuda]

[–timeout ] [–json] [–manifest]
...
–device       cpu|gpu (hint for tiny train smoke; default: cpu)

–log       write a copy of stdout/stderr (default: logs/selftest.log)

–timeout      timeout for sub-steps (default: 180)
...
• Appends a single structured line to logs/v50_debug_log.md (best-effort).
...
JSON_OUT=0
WRITE_MANIFEST=0
STEP_TIMEOUT=”${SELFTEST_TIMEOUT:-180}” # env override
...
Rotate old log (keep last 3)

if [[ -f “$LOG_PATH” ]]; then
mv -f “$LOG_PATH” “${LOG_PATH}.1” 2>/dev/null || true
[[ -f “${LOG_PATH}.1” && -f “${LOG_PATH}.2” ]] && mv -f “${LOG_PATH}.2” “${LOG_PATH}.3” 2>/dev/null || true
fi

Redirect stdout/stderr to tee unless quiet
...
RUN_FAILED=0
trap ‘RUN_FAILED=1; err “Selftest crashed unexpectedly.”; exit 1’ ERR
...
check “Git present” bash -lc ‘command -v git >/dev/null’
GIT_SHA_SHORT=”$(bash -lc ‘git rev-parse –short HEAD 2>/dev/null || echo nogit’)”
json_put “git_sha” “${GIT_SHA_SHORT}”
...
if [[ -n “$PY_VER” ]]; then
MAJOR=”$(printf “%s” “$PY_VER” | sed -E ‘s/[^0-9]([0-9]+).([0-9]+)./\1/’)”
MINOR=”$(printf “%s” “$PY_VER” | sed -E ‘s/[^0-9]([0-9]+).([0-9]+)./\2/’)”
if [[ “$MAJOR” -lt 3 || ( “$MAJOR” -eq 3 && “$MINOR” -lt 10 ) ]]; then
warn “Python ${PY_VER} detected; 3.10+ recommended.”
fi
fi
...
VERSION vs pyproject.toml sync
...
CLI_VERSION=”$(bash -lc “${CLI_BIN} –version 2>/dev/null | head -n1 || echo unknown”)”

Get config hash if supported
...
CLI selftest (fast) — skip if maps back to this script (heuristic: subcommand exists)
...
Light analyze-log (best-effort)

if bash -lc “${CLI_BIN} –help | grep -qi ‘analyze-log’”; then
say “Analyzing CLI log (short)…”
run “analyze-log” bash -lc “${CLI_BIN} analyze-log –no-refresh –tail 20 –md outputs/_selftest_log.md –csv outputs/_selftest_log.csv” || true
fi

Diagnostics dashboard (skippable for quick/CI/Kaggle)

if [[ “$SKIP_HEAVY” -eq 1 || “$IS_KAGGLE” -eq 1 ]]; then
warn “Skipping diagnostics/dashboard per –skip-heavy or Kaggle guard.”
else
...
Optional tiny train smoke (deep only, not Kaggle)

if [[ “$DEEP” -eq 1 && “$IS_KAGGLE” -eq 0 ]]; then
say “Device smoke test (tiny training)…”
run “tiny train (${DEVICE})” bash -lc “${CLI_BIN} train +training.epochs=1 –device ${DEVICE} –outdir outputs/checkpoints/selftest${DEVICE}” || FAILED=1
else
...
json_put “cli_version” “${CLI_VERSION}”
...
RUN_ID=”${START_TS}-${GIT_SHA_SHORT}”
MANIFEST_PATH=“outputs/manifests/selftest_manifest_${RUN_ID}.json”

Small, explicit manifest (no external deps)
...
printf ’    “end_utc”: “%s”,\n’   “$END_TS”
printf ’    “ok”: %s\n’           “$($STATUS_OK && echo true || echo false)”
printf ’  },\n’
printf ’  “env”: {\n’
printf ’    “ci”: %s,\n’          “$([[ $IS_CI -eq 1 ]] && echo true || echo false)”
printf ’    “kaggle”: %s,\n’      “$([[ $IS_KAGGLE -eq 1 ]] && echo true || echo false)”
printf ’    “python”: “%s”,\n’    “${PY_VER:-unknown}”
printf ’    “poetry”: “%s”\n’     “${POETRY_VER:-unknown}”
printf ’  },\n’
printf ’  “cli”: {\n’
...
okmark “Manifest written → ${MANIFEST_PATH}”
fi

––––– Structured line to v50_debug_log.md (best-effort) –––––

DEBUG_LOG=“logs/v50_debug_log.md”
mkdir -p “$(dirname “$DEBUG_LOG”)”
printf ‘%s cmd=%s ok=%s git=%s cfg_hash=%s log=%s\n’ 
“$END_TS” “selftest” “$($STATUS_OK && echo 1 || echo 0)” “$GIT_SHA_SHORT” “${CFG_HASH:-”-”}” “$LOG_PATH” >> “$DEBUG_LOG” || true
