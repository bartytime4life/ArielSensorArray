...
export interface ModalProps {
  ...
  /** Optional description text under the title. If provided and no custom header is used, it will link via aria-describedby. */
  ...
  /** Optional id(s) for aria-labelledby/aria-describedby; autogenerated if omitted */
  labelledById?: string;
...
export const Modal: React.FC<ModalProps> & {
  ...
  const computedLabelId = labelledById ?? `modal-title-${uid}`;
  const computedDescId =
    (description ? describedById ?? `modal-desc-${uid}` : describedById) ?? undefined;
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (open) {
      ...
      // prefer provided ref → element with data-autofocus → first focusable
      const target =
  ...
  React.useEffect(() => {
    ...
    const handleKeydown = (e: KeyboardEvent) => {
      ...
      if (focusables.length === 0) {
        // keep focus on panel if nothing else
        e.preventDefault();
  ...
  const modalContent = (
    <div
      ...
      onMouseDown={(e) => {
        ...
        // only close if click is on overlay (not bubbled from inside panel)
        if (e.target === overlayRef.current) onClose();
      }}
      // prevent scroll chaining (mostly iOS)
      onWheel={(e) => {
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby={title ? computedLabelId : undefined}
      ...
      >
        {/* Default header (rendered when title provided).
            Consumers can also use <Modal.Header> to fully customize */}
        {title !== undefined && (
          <div className="flex items-start justify-between gap-4 p-4 border-b border-gray-200 dark:border-gray-700">
            <div className="min-w-0">
              ...
              {description && (
                <p id={computedDescId} className="mt-1 text-sm text-muted-foreground">
                  {description}
                </p>
              )}
            </div>
