...
@echo "  \$(CYN)repair\$(RST)              : run ./bin/repair\_and\_push.sh MSG="...""
...
predict-e2e: predict
@test -f "\$(PRED\_DIR)/submission.csv" && echo "\$(GRN)OK: \$(PRED\_DIR)/submission.csv\$(RST)" || (echo "\$(RED)Missing submission.csv\$(RST)"; exit 1)
...
base = f"[https://data.pyg.org/whl/torch-{ver}+{'cu'+cu](https://data.pyg.org/whl/torch-{ver}+{'cu'+cu) if torch.version.cuda else 'cpu'}.html"
print(base)
PY
...
@echo ""
@echo "Notes: demos live under \$(GUI\_DIR). They do not mutate the pipeline and only call \$(CLI) + render artifacts."
